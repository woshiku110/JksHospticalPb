...
/**
 * @author xieyan
 * @date 2016-9-19 17:45:58
 * 
 * 说明：
 * 6个常用方法，且都是热响应的
 * @method MsgEngin.start(url) 启动消息推送引擎，开始接收服务器推送的消息
 * @param url 开启引擎的地址
 * 
 * @method MsgEngin.colse() 关闭引擎
 * @method MsgEngin.subscribe(topic) 订阅一个topic(topic是个字符串)
 * @method MsgEngin.unsubscribe(topic) 取消订阅一个topic(topic是个字符串)
 * 
 * @method MsgEngin.addListener(intent,listener(msg)) 给某个意图添加一个监听器
 * @param intent 意图，一个正则表达式，已经加上了起止符，^intent$
 * @param listener 监听器，一个方法，当消息和意图匹配的时候执行该方法,参数是收到的消息对象
 * 
 * @method MsgEngin.removeListener(intent,listener(msg)) 移除某个意图的一个监听器
 * @param intent 意图，一个正则表达式，已经加上了起止符
 * @param listener 监听器，注意函数对象，不是函数名字。为null删除intent
 * 
 * 其他方法
 * @method setRuntimeListener(listener(state)) （热响应的）
 * 	设置监听引擎运行状态改变的方法，state表示引擎运行状态，0停止，1正在启动，2推送，3轮询，4重联推送服务器
 * 
 * @method setPollCyc(ms) 设置引擎以轮询的方式运行的时候，轮询的周期，单位毫秒（可能是非热响应的）
 * @method getPollCyc() 获的引擎以轮询的方式运行的时候的轮询周期
 * 
 */
var MsgEngin = new function(){
	var runtimeState=0;//0停止，1正在启动，2推送，3轮询，4重联推送服务器
	var topiclist = new List();
	var mqObject;
	var intentMap = new Map();
	var intentRegExp = new Map();
	...
	this.start = function(url){
		this.setRuntimeState(1);
		var xmlhttp=ajax();
		xmlhttp.onreadystatechange=function(){
			if(xmlhttp.readyState==4 && xmlhttp.status==200){
				var addr=eval("("+ xmlhttp.responseText +")");
				var clientId = "web"+new Date().getTime();
				mqObject = new Push(clientId,addr);
				mqObject.onConnect();
			}
		};
		xmlhttp.open("POST",url,true);
		xmlhttp.setRequestHeader("X-Requested-With", "xieyan-push-ajax");
		xmlhttp.send("querytime="+new Date().getTime());
	};
	this.close = function(){
		if(runtimeState==2){
			mqObject.disconnect();
	...
	this.subscribe = function(topic){
		...
		topiclist.add(topic);
		if(runtimeState==2){
			mqObject.subscribe(topic);
		}
	};
	this.unSubscribe = function(topic){
		topiclist.removeObj(topic);
		if(runtimeState==2){
			mqObject.unsubscribe(topic);
		}
	};
	this.addListener = function(intent, listener){
		if(intent==null || listener==null)
			return;
		if(typeof(listener) != 'function')
			return;
		intentRegExp.put(intent, new RegExp("^"+intent+"$"));
		var list=intentMap.get(intent);
		if(list == null){
			list=new List();
		}
		list.add(listener);
		intentMap.put(intent, list);
	};
	this.removeListener = function(intent, listener){
		if(listener==null){
			intentMap.remove(intent);
			intentRegExp.remove(intent);
			return;
		}
		var list=intentMap.get(intent);
		if(list == null){
	...
	this.setRuntimeState = function(state){
		runtimeState=state;
		runtimeListener(runtimeState);
	};	
	this.getRuntimeState=function(){
		return runtimeState;
	};
	this.setRuntimeListener = function(listener){
		runtimeListener = listener;
	};
	this.setPollObj = function(obj){
		pollObj = obj;
	};
	this.getPollObj = function(){
		return pollObj;
	};
	this.setPollCyc = function(ms){
		pollCyc = ms;
	};
	this.getPollCyc = function(){
		return pollCyc;
	};
	...
	this.notify = function(msg){
		var entrys = intentRegExp.getEntrys();
		for(var i=0;i<entrys.length;i++){
			if(entrys[i].value.test(msg.intent)){
				var listerers = intentMap.get(entrys[i].key);
				for(var j=0;j<listerers.size();j++){
					(listerers.get(j))(msg);
				}
			}
		}
	...
	function Push(clientId,addr){
		var client;
		this.onConnect = function() {
			client = new Messaging.Client(addr.ip, addr.port, clientId);

			client.onConnect = this.onSuccess;
		...
		this.ubsubscribe = function(topic){
			client.unsubscribe(topic);
		};

		this.onSuccess = function(frame){
			var topiclist = MsgEngin.getTopiclist();
			for(var i=0;i<topiclist.size();i++){
				client.subscribe(topiclist.get(i));
			}
			MsgEngin.setRuntimeState(2);
		};
		
		this.onFailure = function(failure) {
			var p=new Poll(addr);
			MsgEngin.setPollObj(p);
			MsgEngin.sendPollRequest=p.send;
			p.start();
		};
	...
	function Poll(addr){
		var queryObj = ajax();
		var interval = null; //间隔定时器
		var startTime = null;

		this.start = function(){
			startTime = new Date().getTime();
			interval = setInterval("MsgEngin.sendPollRequest()",MsgEngin.getPollCyc());
			interval.send = this.send;
			MsgEngin.setRuntimeState(3);
		};
		...
		queryObj.onreadystatechange=function(){
			if(queryObj.readyState==4 && queryObj.status==200){
				var text = queryObj.responseText;
				var msglist=eval("("+ text +")");
				for(var i=0;i<msglist.length;i++){
					MsgEngin.notify(msglist[i]);
				};
			};
		};	
		...
		this.send = function(){
			...
			queryObj.open("POST",addr.poll,true);
			queryObj.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			var arr = MsgEngin.getTopiclist().getArray();
			var topics = "";
			...
			topics = "["+topics+"]";
			queryObj.setRequestHeader("X-Requested-With", "xieyan-push-ajax");
			queryObj.send("topics="+topics+"&startTime="+startTime+"&endTime="+endTime);
			startTime = endTime;
		};
	};
	this.sendPollRequest=function(){console.log("未实现的轮询请求方法");};
